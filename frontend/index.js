function moduleProject3() {
  ...
  function buildLearnerCard(learner, languages) {
    ...
    favLanguage = favLanguages.find(lang =>lang.id === learner.favLanguage)
    favLangP.textContent =`Favorite Language: ${favLanguage.name}`;

    [nameP,idElement,dobP,favLangP].forEach( card => {
      card.appendChild(p)
    })
  ...
  function buildFooter(footerData) {
    ...
    contactEmailP.innerHTML = `Email: <a href=a"mailto:${footerData.contactEmail}"> ${footerData.contactEmail}</a>`
    ...
    copyright.textContent = `${footerData.companyName. toUpperCase()}`
